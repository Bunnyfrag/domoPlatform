...
// Valeur
	if ( $_REQUEST['action'] == 'add' ){
		$newValeur = $ancienValeur + 1;
	} else {
		$newValeur = $ancienValeur - 1;
	}

// Change the state of the shutters
	if ($_REQUEST['action'] == 'open'){
		$newValeur = 0;	// Opened
	} elseif ($_REQUEST['action'] == 'semi-open'){
		$newValeur = 1;	// Semi-opened
	} else {
		$newValeur = 2; // Closed
	}
...
// Insertion d'une publication en basedu message
	...
	$prep->bindValue( 'message', 			'Le volet est passé de [' . $ancienValeur . '] à [' . $newValeur . ']' );
